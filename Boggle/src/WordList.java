...
public class WordList extends ArrayList<String>
{
    ...
    private double[] charFrequency = new double[26];
            ...
            try
            {
            	...
            	while (scanner.hasNext()) {
	                ...
	                if (word.length() >= min && word.length() <= max) {
	                	...
	                	// loop through each word and find char frequency
	                	for (int i = 0; i < word.length(); i++) {
	                		charFrequency[Character.toUpperCase(word.charAt(i))-65]++;
	                	}
	                }
	            }
            	
            	// find percentage char frequency
            	int total = 0;
            	for (double cfc : charFrequency) { total += cfc; }
					for (int i = 0; i < charFrequency.length; i++) {
						charFrequency[i] = charFrequency[i]/total;
            	}
    ...
    public void Dump() {
        // display the full list in order without duplicates
    ...
    //returns a random letter based on the chance of each letter
    public char nextLetter() {
    	double[] charFrequencyClone = charFrequency.clone();
    	for (int i = 0; i < charFrequencyClone.length; i++) {
    		charFrequencyClone[i] = charFrequencyClone[i]*Math.random();
    	}
    	// get max index
    	int maxIndex = 0;
    	double max = 0;
		for (int i = 0; i < charFrequencyClone.length; i++) {
			if (charFrequencyClone[i] > max) {
				max = charFrequencyClone[i];
				maxIndex = i;
			}
		}
